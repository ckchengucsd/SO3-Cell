...
def build_hierarchy(all_paths, power_start, level=0, verbose=False):
    ...
    for (s_net, e_net) in cuts:
        ...
        child_hierarchy = {}
        unique_transistors = list(sub_transistors_set)
        ...
        if max_length <= 1:
            ...
            for path_tr_list in sub_paths:
                for tr in path_tr_list:
                    ...
                    unique_nets.add(tr[2])
                    unique_nets.add(tr[3])
            ...
            for tr in all_transistors:
                if depth !=0 and len(all_segments) == 1:
                    for key, sub_value in source[depth-1][tr].items():
                        ...
                        model.addConstr(source[depth][tr][key] <= source[depth-1][tr][key],name=f"source_follow_{depth}_{tr}_{key}")
                    model.addConstr(network[seg_name][tr] == gp.quicksum(source[depth][tr][key] for key, sub_value in source[depth-1][tr].items()),name=f"source_bound_{depth}_{tr}_{key}")
                    ...
                    model.addConstr(network[seg_name][tr] == gp.quicksum(drain[depth][tr][key] for key, sub_value in drain[depth-1][tr].items()),name=f"drain_bound_{depth}_{tr}_{key}")
